...
class OthelloState():
    ...
    def apply_move(self, move):
        ...
        ## YOU NEED TO IMPLEMENT THIS SO THAT new_state IS THE STATE AFTER move
        ## HAS BEEN MADE

        # The row and column at which the piece is placed
        row, col = move.pair

        new_state.board[row][col] = move.player

        # Check for a flank in every direction
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                if dr==0 and dc==0:
                    continue

                # If we're flanking a piece in the direction (dr, dc)
                if self.flanking(row + dr, col + dc, dr, dc,
                                 opposite_color(self.current),
                                 self.current):
                    # Get the next r and c values
                    next_r, next_c = row + dr, col + dc

                    # Loop through the direction to change all the pieces
                    while new_state.board[next_r][next_c] != move.player:
                        new_state.board[next_r][next_c] = move.player
                        next_r, next_c = next_r + dr, next_c + dc

        # Assume that the next player has an available move
        ...
        if new_state.available_moves(): # python is weird...
            return new_state

        # The next player has no available moves, so revert the changes
